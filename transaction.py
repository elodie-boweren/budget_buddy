...
class Transaction:
    ...
    def validate_transfer(self):
        try:
            ...
            # Check fields
            if not iban:
                self.error_label.configure(text="Please enter an IBAN")
                return
            ...
            if not dest_account:
                self.error_label.configure(text="Invalid IBAN")
                return
               
            dest_account_id, dest_user_id = dest_account
           
            # Check that it is not a "self transfer"
            if dest_user_id == self.user_id:
                self.error_label.configure(text="You cannot transfer money to yourself")
                return
